...
const Integrations = () => {
    ...
    const handleAddProject = async (e) => {
        e.preventDefault();
        try {
            await axios.post(`${API_BASE_URL}/api/integrations`, { name: newProjectName });
            setNewProjectName('');
            setShowModal(false);
            fetchProjects();
        } catch (error) {
    ...
    const toggleMaintenance = async (id, currentStatus) => {
        try {
            await axios.patch(`${API_BASE_URL}/api/integrations/${id}/maintenance`, { is_maintenance: !currentStatus });
            fetchProjects();
        } catch (error) {
                    ...
                    <div className="bg-dark-panel p-6 rounded border border-dark-border overflow-hidden relative">
                        ...
                        <div className="space-y-3">
                            <div className="bg-dark-bg border border-dark-border rounded p-3">
                                ...
                                <code className="text-neon-green font-mono text-xs block">{API_BASE_URL}/api/v1</code>
                            </div>
